-- ПРОСТОЕ ИСПРАВЛЕНИЕ СИНХРОНИЗАЦИИ
-- Скопируйте и выполните в Supabase SQL Editor

-- 1. Создаем функцию синхронизации
...
-- 3. Синхронизируем ВАШЕГО пользователя
SELECT sync_employee_to_userprofile('b50c566d-8f9c-4c6c-a2e0-7475db3ae7d5');

-- 4. Синхронизируем ВСЕХ пользователей одним запросом
UPDATE user_profiles SET
    ...
    office_name = o.name,
    role = CASE WHEN e.is_admin THEN 'admin' ELSE 'user' END,
    updated_at = NOW()
FROM employees e
LEFT JOIN offices o ON o.id = e.office_id
WHERE user_profiles.id = e.user_id;

-- 5. Создаем триггер для автоматической синхронизации
...
-- 6. Проверяем результат для вашего пользователя
SELECT 
    'РЕЗУЛЬТАТ СИНХРОНИЗАЦИИ:' as status,
    up.full_name,
    up.position,
    up.office_name,
    o.name as office_from_table
