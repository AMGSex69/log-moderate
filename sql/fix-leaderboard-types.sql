-- ===========================================
-- ИСПРАВЛЕНИЕ ТИПОВ ДАННЫХ В ЛИДЕРБОРДЕ
-- ===========================================

-- Исправляем функцию get_leaderboard_stats() с правильными типами
...
BEGIN
    ...
    IF task_logs_exists THEN
        ...
        LEFT JOIN (
            -- Статистика из task_logs с правильными типами
            SELECT 
                tl.employee_id,
    ...
    ELSIF work_sessions_exists THEN
        ...
        SELECT 
            ...
            COALESCE(up."position", 'Сотрудник')::TEXT as employee_position,
            COALESCE(o.name, 'Рассвет')::TEXT as office_name,
            COALESCE(stats.total_sessions, 0)::INTEGER as total_tasks,
            COALESCE(stats.completed_sessions, 0)::INTEGER as completed_tasks,
            COALESCE(
                CASE 
            ...
            COALESCE(stats.total_work_minutes, 0)::INTEGER as total_points,
            COALESCE(up.avatar_url, '')::TEXT
        FROM user_profiles up
        LEFT JOIN employees e ON e.user_id = up.id
        LEFT JOIN offices o ON o.id = up.office_id
        LEFT JOIN (
            -- Статистика из work_sessions с правильными типами
            SELECT 
                ws.employee_id,
                COUNT(*)::INTEGER as total_sessions,
                COUNT(CASE WHEN ws.end_time IS NOT NULL THEN 1 END)::INTEGER as completed_sessions,
                SUM(COALESCE(ws.total_work_minutes, 
                    CASE 
                        ...
                        ELSE 0 
                    END, 0))::INTEGER as total_work_minutes
...
-- Тестируем функцию
SELECT 'Тест функции get_leaderboard_stats с исправленными типами:' as test_type;
SELECT 
    employee_id,
    full_name,
    employee_position,
...
COMMENT ON FUNCTION get_leaderboard_stats() IS 'Функция лидерборда с правильными типами данных (INTEGER вместо BIGINT)'; 