-- ===========================================
-- ИСПРАВЛЕНИЕ ФУНКЦИИ СОТРУДНИКОВ И ВОССТАНОВЛЕНИЕ ПРОФИЛЯ (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- ===========================================
...
-- 4. ИСПРАВЛЕНО: Создаем дополнительных сотрудников только из существующих пользователей
-- Вместо создания новых записей с случайными UUID, используем существующих пользователей
DO $$
DECLARE
    default_office_id INTEGER;
...
BEGIN
    ...
    -- Получаем существующих пользователей без профилей (максимум 3)
    SELECT ARRAY(
        SELECT au.id 
        FROM auth.users au 
        LEFT JOIN user_profiles up ON up.id = au.id 
        WHERE up.id IS NULL 
        AND au.email != 'egordolgih@mail.ru'
        LIMIT 3
    ...
    IF array_length(existing_user_ids, 1) > 0 THEN
        ...
        LOOP
            ...
            INSERT INTO user_profiles (
                ...
                user_id,
                CASE counter
                    WHEN 1 THEN 'Анна Петровна Иванова'
                    WHEN 2 THEN 'Дмитрий Сергеевич Козлов'
                    WHEN 3 THEN 'Елена Владимировна Смирнова'
                    ELSE 'Тестовый пользователь'
                END,
                CASE counter
                    WHEN 1 THEN 'Менеджер по продажам'
                    WHEN 2 THEN 'Ведущий специалист'
                    WHEN 3 THEN 'Аналитик данных'
                    ELSE 'Сотрудник'
                END,
                default_office_id, 
                '5/2', 
            ...
            -- Также создаем запись в employees
        ...
        RAISE NOTICE 'Создано % тестовых профилей из существующих пользователей', counter;
    ELSE
        RAISE NOTICE 'Нет доступных пользователей без профилей для создания тестовых данных';
    END IF;
END $$;
