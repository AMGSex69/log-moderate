-- ИСПРАВЛЕННАЯ ФУНКЦИЯ СТАТИСТИКИ ОФИСА
...
BEGIN
    ...
    LEFT JOIN (
        ...
        FROM employees emp
        WHERE emp.office_id = user_office_id
    ) employees_count ON true
    LEFT JOIN (
        -- Считаем работающих сейчас
        SELECT COUNT(*) as working
        FROM employees emp
        INNER JOIN work_sessions ws ON ws.employee_id = emp.id
        WHERE emp.office_id = user_office_id
    ...
    LEFT JOIN (
        ...
        SELECT 
            ...
            END as avg_hours
        FROM employees emp
        LEFT JOIN work_sessions ws ON ws.employee_id = emp.id AND ws.date = CURRENT_DATE
        WHERE emp.office_id = user_office_id
    ) work_stats ON true
...
-- ТЕСТ ФУНКЦИИ
SELECT 'Исправленная функция создана, тестируем...' as status;

-- Тестируем с фиктивным UUID
SELECT * FROM get_office_statistics('00000000-0000-0000-0000-000000000000'::UUID);

-- Проверяем что офисы существуют
SELECT 'Доступные офисы:' as info;
...
ORDER BY emp.office_id, emp.full_name
LIMIT 10; 