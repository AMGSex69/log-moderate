-- ===========================================
-- БЕЗОПАСНОЕ ИСПРАВЛЕНИЕ ПРОБЛЕМ С НОВЫМИ ПОЛЬЗОВАТЕЛЯМИ
-- ===========================================
...
-- 5. СОЗДАЕМ ФУНКЦИЮ БЕЗОПАСНОГО ПОЛУЧЕНИЯ EMPLOYEE_ID
...
-- 6. ИСПРАВЛЯЕМ ТРИГГЕР С ПРИНУДИТЕЛЬНЫМ УДАЛЕНИЕМ
DO $$
BEGIN
    -- Принудительно удаляем старую функцию со всеми зависимостями
    DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
    
    RAISE NOTICE 'Старая функция handle_new_user и зависимости удалены';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Ошибка при удалении старой функции: %', SQLERRM;
END $$;
