-- ИСПРАВЛЕННАЯ МИГРАЦИЯ К ЕДИНОЙ ТАБЛИЦЕ user_profiles
-- Объединяем user_profiles и employees в одну таблицу
-- ИСПРАВЛЕНО: убраны ссылки на несуществующие поля
...
-- Добавляем недостающие поля из employees (только существующие)
...
-- ===========================================
-- ШАГ 4: СИНХРОНИЗАЦИЯ ДАННЫХ (ИСПРАВЛЕНО)
-- ===========================================

BEGIN;

-- Обновляем user_profiles данными из employees (только существующие поля)
...
-- Создаем записи user_profiles для employees без профиля (исправлено)
INSERT INTO user_profiles (
    id, full_name, position, work_schedule, work_hours, 
    office_id, is_admin, avatar_url, email, is_active, 
...
-- Обновляем другие таблицы (если существуют)
DO $$
BEGIN
    -- break_logs
...
-- Для других таблиц (если существуют)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'break_logs') THEN
...
-- ===========================================
-- ШАГ 10: УДАЛЕНИЕ СТАРЫХ ФУНКЦИЙ
-- ===========================================
...
-- ВНИМАНИЕ: После проверки результатов можно удалить таблицу employees
